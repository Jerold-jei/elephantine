...
try {
    ...
    $mail->Username   = 'elephantine03@gmail.com';                     //SMTP username
    $mail->Password   = 'clqbhoaldixeqojx';                               //SMTP password
    $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
    $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

    //Recipients
    $mail->setFrom('elephantine03@gmail.com', 'Server');
    $mail->addAddress('sales@elephantine.co', 'Admin');     //Add a recipient

    //Content
    $mail->isHTML(true);                                  //Set email format to HTML
    $mail->Subject = 'Aggregation Form';
    ...
    $message .= "<tbody>
          ...
          <tr align='left' height='50'>
          <th style='font-size:15px; border: 1px groove; padding-left: 15px; font-family: Roboto-Regular;'>Subject</th>
          <td style='font-size:15px; border: 1px groove; padding-left: 15px; font-family: Roboto-Regular;'>".$subject."</td>         
          </tr>    
    ...
    $sql = "INSERT INTO `aggregation_form`(`name`, `email`, `phone`, `subject`, `message`) VALUES ('$name','$email','$phone','$subject','$comment')";

    if ((mysqli_query($conn, $sql))&&($mail->send())) {
        echo "<script> 
        alert('Form is submitted successfully');
        window.location.href = 'aggregation.php';
        </script>";
    } else {
